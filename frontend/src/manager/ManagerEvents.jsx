...
const mockPendingProposals = [
  ...
  },
  {
    ...
    description: "Mettre en place des arceaux pour garer les vélos en toute sécurité.",
  ...
  const handleAction = async (id, action) => {
    ...
    if (window.confirm(`Êtes-vous sûr de vouloir ${actionText} cette proposition citoyenne ?`)) {
      ...
      try {
        ...
        alert(`La proposition a été ${action === 'approve' ? 'validée et publiée pour le vote' : 'refusée'}.`);
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Examinez les idées d'aménagement soumises par les citoyens de votre arrondissement.
        </p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-100 border-b border-gray-300 text-sm text-gray-700 uppercase">
              <th className="p-3 border-r border-gray-200 w-1/3">Idée & Description</th>
              <th className="p-3 border-r border-gray-200">Localisation</th>
              <th className="p-3 border-r border-gray-200">Citoyen</th>
              <th className="p-3 border-r border-gray-200">Date de soumission</th>
          ...
          <tbody>
            {proposals.length === 0 ? (
              <tr>
                <td colSpan="5" className="p-6 text-center text-gray-500 font-bold bg-gray-50/50">
                  <span className="material-symbols-outlined text-3xl block mb-2 text-gray-400">task_alt</span>
                  Aucune proposition en attente de validation dans votre secteur.
                </td>
              </tr>
            ) : (
              proposals.map(prop => (
                <tr key={prop.id} className="border-b border-gray-200 hover:bg-gray-50 text-sm text-gray-800 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        <span className="material-symbols-outlined text-[14px]">close</span> Refuser
                      </button>
                    </div>
                  </td>
