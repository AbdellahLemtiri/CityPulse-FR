...
// هادا هو الـ Array لي غاتبدل فيه التصاور والسميات ديال مقاطعات آسفي (Statique دابا)
const secteursSafi = [
    { id: 'plateau', nom: 'Quartier Plateau', img: 'https://images.unsplash.com/photo-1517048676732-d65bc937f952?q=80&w=200&auto=format&fit=crop', lat: '32.2994', lng: '-9.2372' },
    { id: 'medina', nom: 'Médina Ancienne', img: 'https://images.unsplash.com/photo-1539020140153-e479b8c22e70?q=80&w=200&auto=format&fit=crop', lat: '32.3008', lng: '-9.2272' },
    { id: 'sidi_bouzid', nom: 'Sidi Bouzid', img: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=200&auto=format&fit=crop', lat: '32.3167', lng: '-9.2500' },
    { id: 'kaoki', nom: 'KaoKi / Anas', img: 'https://images.unsplash.com/photo-1480714378408-67cf0d13bc1b?q=80&w=200&auto=format&fit=crop', lat: '32.2833', lng: '-9.2167' }
];
...
export default function Register() {
    ...
    // الـ State ديال قوة المودباس (باش نلونو الشريط)
    const [pwdStrength, setPwdStrength] = useState({ score: 0, length: false, upper: false, number: false, special: false });
    ...
    const handleChange = (e) => {
        ...
        // يلا كان كييقاد المودباس، نفحصو القوة ديالو فالبلاصة
        if (id === 'password') checkPasswordStrength(value);
    };

    // هادي كتحسب شحال المودباس قوي 
    const checkPasswordStrength = (pwd) => {
        ...
        let score = (reqs.length ? 1 : 0) + (reqs.upper ? 1 : 0) + (reqs.number ? 1 : 0) + (reqs.special ? 1 : 0);
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        if (pwdStrength.score < 3 || !formData.terms) return; // تأكد أخير
        setErrors(null);
        setLoading(true);
            ...
            <main className="flex-grow w-full max-w-lg z-10 flex flex-col">
                ...
                <div className="w-full bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-2xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-slate-800 relative min-h-[400px]">
                    <form onSubmit={onSubmit} className="flex flex-col h-full justify-between">
                        ...
                        {/* ---------------- ÉTAPE 2: Localisation (بـ التصاور ديال المقاطعات) ---------------- */}
                        {step === 2 && (
                            <div className="animate-fade-in flex flex-col h-full justify-between space-y-4">
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-4 max-h-[220px] overflow-y-auto pr-1">
                                        {secteursSafi.map((sec) => (
                                            <div 
                                                key={sec.id} 
                                                onClick={() => setFormData({...formData, quartier: sec.nom})}
                                                className={`relative cursor-pointer rounded-xl overflow-hidden border-2 transition-all ${formData.quartier === sec.nom ? 'border-orange-600 shadow-md scale-[1.02]' : 'border-transparent hover:border-gray-300'}`}
                                            >
                                                <img src={sec.img} alt={sec.nom} className="w-full h-20 object-cover brightness-75" />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-2">
                                                    <span className="text-white text-xs font-bold">{sec.nom}</span>
                                                    <span className="text-[9px] text-gray-300 flex items-center gap-1"><span className="material-symbols-outlined text-[10px]">location_on</span> {sec.lat}, {sec.lng}</span>
                                                </div>
                                                {formData.quartier === sec.nom && (
                                ...
                                <div className="flex gap-3 mt-4">
                                    <button type="button" onClick={prevStep} className="px-6 border border-gray-200 dark:border-slate-700 text-gray-500 font-bold py-4 rounded-xl hover:bg-gray-50 transition-all">Retour</button>
                                    <button type="button" onClick={nextStep} disabled={!formData.quartier} className="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                                        Suivant <span className="material-symbols-outlined text-sm">arrow_forward</span>
                                    </button>
                                </div>
                            </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-1 relative">
                                            ...
                                            {formData.password.length > 0 && (
                                                <div  className="bg-slate-50 dark:bg[0c4a6e] mt-3 bg-gray-50 p-3 rounded-lg  ">
                                                    <div className="flex justify-between mb-1">
                                                        <span className="text-[10px] uppercase font-bold text-gray-500">Force</span>
                                                        <span className={`text-[10px] font-bold ${pwdStrength.score >= 3 ? 'text-green-600' : 'text-orange-600'}`}>{strengthLabels[pwdStrength.score]}</span>
                                                    </div>
                                                    <div className="flex gap-1 h-1.5 mb-2">
                                                        {[1, 2, 3, 4].map(num => (
                                                            <div key={num} className={`flex-1 rounded-full ${pwdStrength.score >= num ? strengthColors[pwdStrength.score] : 'bg-gray-200'}`}></div>
                                                        ))}
                                                    </div>
                                                    <div className=" grid grid-cols-2 gap-1 text-[10px] text-gray-500">
                                                        <div className={`flex items-center gap-1 ${pwdStrength.length ? 'text-green-600' : ''}`}><span className="material-symbols-outlined text-[12px]">{pwdStrength.length ? 'check' : 'close'}</span> 8 caractères</div>
                                                        <div className={`flex items-center gap-1 ${pwdStrength.upper ? 'text-green-600' : ''}`}><span className="material-symbols-outlined text-[12px]">{pwdStrength.upper ? 'check' : 'close'}</span> 1 Majuscule</div>
                                                        <div className={`flex items-center gap-1 ${pwdStrength.number ? 'text-green-600' : ''}`}><span className="material-symbols-outlined text-[12px]">{pwdStrength.number ? 'check' : 'close'}</span> 1 Chiffre</div>
                                                        <div className={`flex items-center gap-1 ${pwdStrength.special ? 'text-green-600' : ''}`}><span className="material-symbols-outlined text-[12px]">{pwdStrength.special ? 'check' : 'close'}</span> Spécial (!@#...)</div>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-start gap-3 pt-2">
                                            <input type="checkbox" id="terms" checked={formData.terms} onChange={handleChange} required className="mt-1 accent-orange-600 w-4 h-4 rounded cursor-pointer" />
                                            <label htmlFor="terms" className="text-xs text-gray-500 leading-snug cursor-pointer select-none">
                                                J'accepte les <a href="#" className="text-orange-600 font-bold hover:underline">conditions d'utilisation</a> de SafiPulse et le traitement de mes données. *
                                            </label>
                                        </div>
                                    </div>
                                </div>
