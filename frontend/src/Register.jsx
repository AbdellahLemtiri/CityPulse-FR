                      ...
                      {formData.password.length > 0 && (
                        <div className="mt-3 p-3 rounded-lg bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700">
                          <div className="flex justify-between mb-1">
                            <span className="text-[10px] uppercase font-bold text-gray-500 dark:text-gray-400">Force</span>
                            <span className={`text-[10px] font-bold ${pwdStrength.score >= 3 ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>{strengthLabels[pwdStrength.score]}</span>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-1 text-[10px] text-gray-500 dark:text-gray-400">
                            <div className={`flex items-center gap-1 ${pwdStrength.length ? 'text-green-600 dark:text-green-400' : ''}`}>
                              <span className="material-symbols-outlined text-[12px]">{pwdStrength.length ? 'check' : 'close'}</span> 8 caractères
                            </div>
                            <div className={`flex items-center gap-1 ${pwdStrength.upper ? 'text-green-600 dark:text-green-400' : ''}`}>
                              <span className="material-symbols-outlined text-[12px]">{pwdStrength.upper ? 'check' : 'close'}</span> 1 Majuscule
                            </div>
                            <div className={`flex items-center gap-1 ${pwdStrength.number ? 'text-green-600 dark:text-green-400' : ''}`}>
                              <span className="material-symbols-outlined text-[12px]">{pwdStrength.number ? 'check' : 'close'}</span> 1 Chiffre
                            </div>
                            <div className={`flex items-center gap-1 ${pwdStrength.special ? 'text-green-600 dark:text-green-400' : ''}`}>
                              <span className="material-symbols-outlined text-[12px]">{pwdStrength.special ? 'check' : 'close'}</span> Spécial (!@#...)
                            </div>
                          </div>
                        </div>
                      )}
