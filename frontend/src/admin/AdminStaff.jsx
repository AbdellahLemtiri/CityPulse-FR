...
const mockStaff = [
  { id: 1, first_name: "Hassan", last_name: "Alami", email: "hassan.manager@safipulse.ma", role: "Manager", sector_id: 1, sector_name: "Quartier Plateau", is_active: true },
  { id: 2, first_name: "Kamal", last_name: "Radi", email: "kamal.presse@safipulse.ma", role: "Journaliste", sector_id: null, sector_name: "-", is_active: true },
  { id: 3, first_name: "Nadia", last_name: "Fathi", email: "nadia.manager@safipulse.ma", role: "Manager", sector_id: 2, sector_name: "Ville Nouvelle", is_active: false },
];
...
export default function AdminStaff() {
  ...
  const handleOpenCreate = () => {
    setFormData({ id: null, first_name: "", last_name: "", email: "", password: "", role: "Manager", sector_id: "" });
    setIsEditing(false);
    setIsModalOpen(true);
  };
  ...
  const handleOpenEdit = (staff) => {
    setFormData({
      ...
      email: staff.email,
      password: "", // On ne remplit pas le mot de passe existant
      role: staff.role,
      sector_id: staff.sector_id || ""
    });
    setIsEditing(true);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e) => {
    ...
    setIsSubmitting(true);
     ...
     setTimeout(() => {
      const sectorName = formData.role === "Manager" 
        ? mockSectors.find(s => s.id === parseInt(formData.sector_id))?.name 
        : "-";

      if (isEditing) {
        setStaffList(staffList.map(s => 
          s.id === formData.id ? { ...s, ...formData, sector_name: sectorName } : s
        ));
        ...
        alert("Nouveau membre du staff créé avec succès !");
      }
  ...
  const toggleStatus = (id) => {
    if (window.confirm("Voulez-vous vraiment modifier le statut de cet accès ?")) {
      setStaffList(staffList.map(s => s.id === id ? { ...s, is_active: !s.is_active } : s));
    }
  };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white border border-gray-300 shadow-sm overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {staffList.map(staff => (
              <tr key={staff.id} className={`border-b border-gray-200 text-sm ${!staff.is_active ? 'bg-red-50/50' : 'hover:bg-gray-50'}`}>
                <td className="p-3 border-r border-gray-200 font-bold text-gray-900">{staff.first_name} {staff.last_name}</td>
                <td className="p-3 border-r border-gray-200 text-gray-600">{staff.email}</td>
                <td className="p-3 border-r border-gray-200">
                  <span className={`px-2 py-0.5 text-[10px] font-bold uppercase border ${
                    staff.role === 'Manager' ? 'bg-purple-100 text-purple-700 border-purple-300' : 'bg-indigo-100 text-indigo-700 border-indigo-300'
                  }`}>
                    {staff.role}
                  </span>
                </td>
                ...
                <td className="p-3 border-r border-gray-200 text-center">
                  {staff.is_active ? (
                    <span className="text-green-600 font-bold uppercase text-[10px] bg-green-100 border border-green-300 px-2 py-1">Actif</span>
                  ) : (
                    <span className="text-red-600 font-bold uppercase text-[10px] bg-red-100 border border-red-300 px-2 py-1">Inactif</span>
                  )}
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white border border-gray-400 w-full max-w-2xl flex flex-col shadow-2xl">
            ...
            <div className="bg-gray-800 p-4 border-b border-gray-900 flex justify-between items-center text-white">
              ...
              <button onClick={() => setIsModalOpen(false)} className="hover:text-gray-300 font-bold px-2">X</button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <select 
                    value={formData.role}
                    onChange={(e) => {
                      setFormData({...formData, role: e.target.value, sector_id: e.target.value === "Journaliste" ? "" : formData.sector_id});
                    }}
                    className="w-full border border-gray-300 p-2 text-sm bg-white focus:outline-none focus:border-blue-500"
                  >
                    <option value="Manager">Manager (Opérateur de secteur)</option>
                    <option value="Journaliste">Journaliste (Narrateur)</option>
                  </select>
                </div>

                {/* Choix du Secteur (Scope Géographique) - Conditionnel */}
                <div className={`transition-opacity ${formData.role === 'Manager' ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">
                  ...
                  {formData.role === 'Journaliste' && (
                    <p className="text-[10px] text-gray-500 mt-1 italic">Un journaliste a un accès global à la ville.</p>
                  )}
                </div>
              </div>
