      ...
      <div className="    p-5 rounded-xl mb-6  flex flex-col sm:flex-row gap-4 items-end">
        <div className="flex-1 w-full">
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Tapez un Nom, Email ou CIN..." className="w-full border border-gray-600 pl-10 p-2.5 text-sm bg-gray-900 text-white rounded-lg focus:outline-none focus:border-red-500 " />
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-gray-900 border-b border-gray-700 text-xs text-gray-400 uppercase ">
              ...
              <th className="p-4 border-r border-gray-700">Email / Contact</th>
              <th className="p-4 border-r border-gray-700 text-center">CIN</th>
              <th className="p-4 border-r border-gray-700 text-center">Niveau d'Infraction</th>
              <th className="p-4 border-r border-gray-700 text-center">Statut du Compte</th>
            ...
            {isLoading ? (
              ...
              filteredUsers.map((user) => (
                <tr key={user.id} className="border-b border-gray-700 hover:bg-gray-700/30 text-sm ">
                  ...
                  <td className="p-4 border-r border-gray-700 text-gray-400">
                    {' '}
                    {user.email} {user.phone}
                  </td>
                  <td className="p-4 border-r border-gray-700 text-center font-medium text-gray-300">{user.cin}</td>

                  <td className="p-4 border-r border-gray-700 text-center">
                    <span className={`font-bold px-2.5 py-1 rounded    text-[11px] uppercase `}>{user.strikes_count} / 3 Strikes</span>
                  </td>
                  ...
                  <td  className="p-4 text-center ">
                    ...
                    <button onClick={() => setUserToStrike(user)} disabled={user.is_banned} className={`px-4 py-1.5 rounded text-xs font-bold uppercase  flex items-center justify-center gap-1 mx-auto ${user.is_banned ? 'bg-gray-900 border border-gray-700 text-gray-600 cursor-not-allowed' : 'bg-red-900/30 hover:bg-red-600 text-red-400 hover:text-white border border-red-800/50 hover:border-red-600'}`}>
                      <span className="material-symbols-outlined text-[16px]">{user.is_banned ? 'lock' : 'warning'}</span>
                      {user.is_banned ? 'Suspendu' : ' Strike'}
                    </button>

                    <button onClick={() => setUserToStrike(user)}  className={`px-4 py-1.5 rounded text-xs font-bold uppercase  flex items-center justify-center gap-1 mx-auto ${user.is_banned ? 'bg-gray-900 border border-gray-700 text-gray-600 cursor-not-allowed' : 'bg-red-900/30 hover:bg-red-600 text-red-400 hover:text-white border border-red-800/50 hover:border-red-600'}`}>
                      <span className="material-symbols-outlined text-[16px]">{user.is_banned ? 'lock' : 'warning'}</span>
